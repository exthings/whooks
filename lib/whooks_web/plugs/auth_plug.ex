defmodule WhooksWeb.Plugs.Auth do
  ...
  def log_in_consumer(conn, consumer, params \\ %{}) do
    conn
    |> create_or_extend_session(consumer, params)
  end
  ...
  def log_out_user(conn) do
    ...
    if live_socket_id = get_session(conn, :live_socket_id) do
      WhooksWeb.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end
  ...
  def fetch_current_scope_for_user(conn, _opts) do
    Logger.info("PLUG: fetching scope for user")

    with {token, conn} <- ensure_user_token(conn),
         {user, token_inserted_at} <- Auth.get_user_by_session_token(token) do
      conn
      |> assign(:current_scope, Scope.for_user(user))
      |> assign_prop(:current_scope, Whooks.Serializer.to_map(Scope.for_user(user)))
  ...
  def fetch_current_scope_for_consumer(conn, _opts) do
  ...
  defp create_or_extend_session(conn, %User{} = user, params) do
  ...
  def require_sudo_mode(conn, _opts) do
    if Auth.sudo_mode?(conn.assigns.current_scope.user, -10) do
      conn
    else
      conn
