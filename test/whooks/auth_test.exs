defmodule Whooks.AuthTest do
  ...
  describe "register_user/1" do
    ...
    test "validates email uniqueness" do
      ...
      {:error, changeset} = Auth.create_user(%{email: email, role: "root"})
      assert "has already been taken" in errors_on(changeset).email

      # Now try with the uppercased email too, to check that email case is ignored.
      {:error, changeset} = Auth.create_user(%{email: String.upcase(email), role: "root"})
      assert "has already been taken" in errors_on(changeset).email
    end

    test "registers users without password" do
      email = unique_user_email()
      {:ok, user} = Auth.create_user(valid_user_attributes(email: email))
